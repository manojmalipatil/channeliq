...
def parse_kpi_from_response(text: str) -> Tuple[str, str]:
    kpi_label = "escalated" # Default
    match = re.search(r'\[KPI:(resolved|escalated|dropped|converted)\]', text)
    if match:
        kpi_label = match.group(1)
        # Strip the KPI line from the visible reply
        text = re.sub(r'\[KPI:(resolved|escalated|dropped|converted)\].*(\n|$)', '', text, flags=re.MULTILINE).strip()
    return text, kpi_label
